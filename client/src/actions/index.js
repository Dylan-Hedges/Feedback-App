...
export const fetchUser = () => async dispatch => {
	//Makes AJAX request for user id and saves it to "res" (note this is the output from Axios)
	const res = await axios.get('/api/current_user');
	//Once request is completed, dispatches the action to reducers with the payload of res (id, if found), ".data" contains the id's which is the only thing we care about (the rest is not needed data)
